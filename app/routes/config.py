...
def _load_store() -> dict[str, dict[str, str]]:
    fallback_path = _store_path()
    return load_profile_store_from_db(db.engine, fallback_path=fallback_path)


def _save_store(store: dict[str, dict[str, str]]) -> None:
    save_profile_store_to_db(db.engine, store)
    export_profile_store_to_path(_store_path(), store)
    export_secret_store_to_path(Path(current_app.instance_path) / 'secrets.local.json', store)
