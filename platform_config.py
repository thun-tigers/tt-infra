...
def profile_sections(profile: str, version: str | None = None, include_image_tags: bool = False, overrides: dict[str, str] | None = None) -> tuple[EnvSection, ...]:
    if profile == 'local':
        ...
        sections = beta_env_sections(version or '0.1.17')
...
def local_env_sections(version: str | None = None, include_image_tags: bool = False) -> tuple[EnvSection, ...]:
    sections: list[EnvSection] = [
        section(
            ...
            entry('CADDY_CONFIG_DIR', 'caddy-local', required=False),
        ...
        section(
            ...
            # local bindet ./instance nach /app/instance (docker-compose.local.yml),
            # nicht die benannten Volumes unter /backup-sources wie beta/production.
            entry('MEMBERS_INSTANCE_DIR', '/app/instance/tt-members-instance'),
            entry('ANALYTICS_UPLOAD_ROOT', '/app/instance/tt-analytics-uploads'),
        ),
        section(
            'Auth',
...
def beta_env_sections(version: str) -> tuple[EnvSection, ...]:
    sections: list[EnvSection] = [
        section(
            ...
            entry('CADDY_CONFIG_DIR', 'caddy', required=False),
            entry('PROXY_PORT', '80'),
...
def production_env_sections(version: str) -> tuple[EnvSection, ...]:
    sections: list[EnvSection] = [
        section(
            ...
            entry('CADDY_CONFIG_DIR', 'caddy-prod', required=False),
            entry('PROXY_PORT', '80'),
            entry('DATABASE_SUFFIX', '', required=False),
            entry('TZ', 'Europe/Zurich'),
            entry('LOG_LEVEL', 'INFO'),
